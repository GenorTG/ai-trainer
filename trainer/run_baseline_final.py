...
def eval_winogrande(item):
    s = item["sentence"]
    a = item["answer"]
    o1, o2 = item["option1"], item["option2"]
...
for n, r in results.items():
    print(f"  {n}: {r['accuracy']}% ({r['correct']}/{r['total']})")
print(f"\nOverall: {tc}/{tq} = {ov}%")
with open("baselines/phi4_baseline_final.json", "w") as f:
    json.dump(
        {"model": "Phi-4 14B Q4_K_M", "n_ctx": N_CTX, "results": results, "overall": ov},
        f,
        indent=2,
    )
